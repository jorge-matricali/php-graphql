...
abstract class ScalarType extends Type implements OutputType, InputType, LeafType, NamedType
{
     ...
     */
    public astNode;
    function __construct(array config = []) -> void
    {
        let this->name =  isset config["name"] ? config["name"]  : this->tryInferName();
        let this->description =  isset config["description"] ? config["description"]  : this->description;
