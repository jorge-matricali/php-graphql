...
class StandardServer
{
     ...
     */
    public static function send500Error(error, bool debug = false, bool exitWhenDone = false) -> void
    {
        var response, helper;
    
        let response =  ["errors" : [FormattedError::createFromException(error, debug)]];
    ...
    public function handleRequest(parsedBody = null, bool exitWhenDone = false) -> void
    {
        ...
        let result =  this->executeRequest(parsedBody);
    ...
    {
        if parsedBody === null {
            let parsedBody =  this->helper->parseHttpRequest();
        }
     ...
     */
    public function processPsrRequest(<ServerRequestInterface> request, <ResponseInterface> response, <StreamInterface> writableBodyStream)
    {
        var result;
    
        let result =  this->executePsrRequest(request);
    ...
    public function executePsrRequest(<ServerRequestInterface> request)
    {
        ...
        let parsedBody =  this->helper->parsePsrRequest(request);
